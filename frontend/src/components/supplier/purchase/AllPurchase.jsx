...
const AllPurchase = () => {
  ...
  const fetchPendingPurchases = async () => {
    try {
      ...
      const response = await All_Api.getPurchaseConfirm();
      const pending = (response || [])
      setPurchases(pending);
      console.log('Fetched pending purchases:', pending);
    } catch (error) {
              ...
              <tbody className="divide-y divide-gray-200">
                {purchases.map((purchase) => (
                  <tr key={purchase.purchaseID} className="hover:bg-gray-50">
                    <td className="px-4 py-3">{purchase.purchaseID}</td>
                    <td className="px-4 py-3">Nguyễn Nhật Trường</td>
                    ...
                    <td className="px-4 py-3">
                      {Number(purchase.total_amount)?.toLocaleString()} VNĐ
                    </td>
                    <td className="px-4 py-3">
                      <Link
                        to={`/supplier/purchase/confirm/${purchase.purchaseID}`}
