...
export const InvoiceOrder = () => {
  ...
  const fetchInvoices = async () => {
    try {
      const response = await All_Api.getInvoicesOrder();
  ...
  const handleDeleteInvoice = async (id) => {
    ...
    setLoading(true);
    try {
      await All_Api.deleteInvoiceOrder(id);
  ...
  return (
    <div className="p-6 bg-white rounded-2xl shadow-lg">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Danh sách hóa đơn bán</h1>
          ...
          <tbody>
            {invoices.map((invoice, index) => (
              ...
              >
                ...
                <td className="py-3 px-4 border-b text-gray-700">{invoice.order}</td>
        ...
        {invoices.map((invoice, index) => (
          ...
          >
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm text-gray-500">Mã: {invoice.order}</span>
            </div>
