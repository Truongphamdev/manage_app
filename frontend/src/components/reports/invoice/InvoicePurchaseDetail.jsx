...
export const InvoicePurchaseDetail = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchInvoiceDetail = async () => {
      try {
        const response = await All_Api.getInvoicePurchaseById(id);
        setInvoice(response);
  ...
  const handleExportInvoice = async (invoiceId) => {
    try {
        const response = await All_Api.exportInvoicePurchase(invoiceId);
        const url = window.URL.createObjectURL(new Blob([response]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', `hoa_don_mua_${invoiceId}.docx`);
        document.body.appendChild(link);
        console.log("Xuất hóa đơn mua:", response);
        link.click();
  ...
  return (
    <div className="p-8 bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-xl max-w-5xl mx-auto border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white border rounded-xl p-5 shadow-sm">
          <h2 className="flex items-center gap-2 text-lg font-semibold mb-3 text-gray-800">
            <User className="w-5 h-5 text-green-500" /> Nhà cung cấp
          </h2>
          <div className="space-y-2 text-gray-700">
            <p><strong>Tên:</strong> {invoice.supplier_name}</p>
            <p><strong>Email:</strong> {invoice.supplier_email}</p>
            <p><strong>Điện thoại:</strong> {invoice.supplier_phone}</p>
            <p><strong>Địa chỉ:</strong> {invoice.supplier_address}</p>
            <p><strong>Công ty:</strong> {invoice.supplier_company}</p>
          </div>
        </div>
      </div>
          ...
          <table className="min-w-full border-collapse">
            <thead className="bg-blue-100 text-blue-700">
              <tr>
                ...
                <th className="py-3 px-4 text-center font-semibold">Giá nhập</th>
